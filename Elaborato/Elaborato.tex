...
>>>>>>> 9ef490d2c85f06d9458073c47c3901da5ea75882
	...
	\section{Dataset} 
		...
		\subsection{Dataset sugli incidenti stradali}
			...
			\end{figure}	
			\imgh{Scatter plot di ogni combinazione di feature dataset sugli incidenti. Nella diagonale della matrice è tracciata la distribuzione della feature divisa per classe.}{0.25}{scatter_incidenti.png}{scatter_incidenti}
 				
	\section{Classificazione e reingegnerizzazione dei dati} 
		Dopo aver dato uno sguardo generale ai dati con cui avremo a che fare possiamo iniziare a fare esperimenti di classificazione e reingegnerizzazione dei dati. Inizialmente mostreremo i primi esperimenti utilizzati per familiarizzare con \texttt{MLPClassifier} con l'iris dataset, per poi passare al dataset sugli incidenti stradali. 
		
		\subsection{Iris Dataset}
			Il primo esperimento di classificazione fatto con questo dataset è stato quello di provare a creare un percettrone multistrato con degli iperparametri messi casualmente, di allenarlo con un insieme di train per poi testarlo con un insieme di test. Non avendo 			

\begin{lstlisting}[language=Python, caption=Primo esperimento con l'iris dataset]
